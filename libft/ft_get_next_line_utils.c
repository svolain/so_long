...
/*   Updated: 2024/01/31 10:00:09 by vsavolai         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_gnl_strjoin(char *s1, char *s2)
{
	...
	ft_gnl_strcpy(str + ft_gnl_strlen(s1), s2);
	free(s1);
	return (str);
}
...
char	*ft_gnl_substr(char *s, unsigned int start, size_t	len)
{
	...
	if (len > ft_gnl_strlen(&s[start]))
		sub = (char *)malloc(sizeof(char) * (ft_gnl_strlen(&s[start]) + 1));
	else
